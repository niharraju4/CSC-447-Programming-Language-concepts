...
object fp1:
  ...
  // use (1 max 2).
  def maxList(xs: List[Int]): Int = xs match {
    case Nil => throw new java.util.NoSuchElementException
    case List(x: Int) => xs(0)
    case v1::v2::remainder => maxList ( { if v1 > v2 then v1 else v2 } :: remainder )
  }
    // TODO: Provide definition here.
  ...
  def maxTailAux(xs: List[Int], accumulator: Int): Int = {
    xs match {
      case Nil => accumulator
      case y::ys => maxTailAux(ys, y max accumulator)
    }
  }
    // TODO: Provide definition here.
  ...
  def otpu(start: Int, end: Int): List[Int] = {
    if start < end then Nil else start :: otpu(start -1, end)
  }
