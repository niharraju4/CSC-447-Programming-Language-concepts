...
object fp3:
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] = {
    ...
    def rmd[X](tmp: X, ys: List[X], count: Int): List[(Int, X)] = {
      ys match {
        case Nil => if (tmp != Nil) then (count, tmp) :: Nil else Nil
        case w :: ws => if (w != tmp) then (count, tmp) :: rmd(w, ws, 1) else  rmd(w, ws, count + 1)
      }
    }
  ...
  def allDistinct(xs: List[Int]): Boolean = {
    xs match {
      case Nil => return true
      case x :: xs => if (member(x, xs)) then false else allDistinct(xs)
    }
  }
    // TODO: Provide definition here.
